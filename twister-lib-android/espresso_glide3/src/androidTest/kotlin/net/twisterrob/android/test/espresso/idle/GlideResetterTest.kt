...
class GlideResetterTest {
	...
	@Test fun testResetChangesApplicationManager() {
		val context = ApplicationProvider.getApplicationContext<Context>()
		val first = RequestManagerRetriever.get().get(context)

		val second = RequestManagerRetriever.get().get(context)
		assertSame(first, second)

		GlideResetter.resetGlide(context)

		val third = RequestManagerRetriever.get().get(context)
		assertNotSame(first, third)
	}
	...
	@Test fun testWithLoadWithResetWhileRunning() {
		launchActivity<TestGlideActivity>().use { scenario ->
			scenario.onActivity { Glide.with(it) } // Ensure Glide has the activity set up.
